...
namespace AccAdm
{
    public partial class RptWebFax : DevExpress.XtraReports.UI.XtraReport
    {
        public RptWebFax()
        {
            InitializeComponent();
        ...
        public RptWebFax(DataRow row, Dictionary<string, Image> dicImage)
        {
            InitializeComponent();
            ...
            LblGubun1.Text = row["GUBUN1"]?.ToString();
            LblJ_BNum.Text = row["J_BNUM"]?.ToString();
            LblGumsu_Serial.Text = row["EMP_NM"]?.ToString();
            ...
            LblGubun1_2.Text = row["GUBUN2"]?.ToString();
            LblChagam.Text = row["LOSS"]?.ToString();
